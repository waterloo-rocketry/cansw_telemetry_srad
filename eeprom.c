...
__EEPROM_DATA(
    (EEPROM_DEFAULT_FREQ >>  0) & 0xFF,
    (EEPROM_DEFAULT_FREQ >>  8) & 0xFF,
    (EEPROM_DEFAULT_FREQ >> 16) & 0xFF,
    (EEPROM_DEFAULT_FREQ >> 24) & 0xFF,
    EEPROM_DEFAULT_POWER,
    0,0,0
);
...
bool eeprom_set_frequency(uint32_t frequency) {
    if(
        frequency >= 902000 &&
        frequency <= 928000 &&
        eeprom_get_frequency() != frequency
    ) {
...
uint32_t eeprom_get_frequency(void) {
    uint32_t eeprom_frequency =
        ((uint32_t) eeprom_read2(EEPROM_ADDR_FREQ+0) <<  0) |
