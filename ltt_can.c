...
static void can_msg_handler(const can_msg_t *msg) {
	...
	if (msg_type == MSG_TELEMETRY_STATE_SWITCH) {
		return; // this should be internal only
	}
	...
	switch (msg_type) {
		...
		case MSG_CONFIG_SET: {
			...
			if (get_config_set_target_board(msg, &board_type, &board_inst) == W_SUCCESS &&
...
void CAN_send_messages(void) {
	...
	if (now - last_transmit > CAN_MESSAGE_PERIOD_MS) {
		...
		build_analog_sensor_16bit_msg(
			PRIO_LOW, (uint16_t)now, SENSOR_12V_CURR, current_sense_val, &msg);
		CAN_enqueue(&msg);

		build_general_board_status_msg(
			error_bitfield ? PRIO_HIGH : PRIO_LOW, (uint16_t)now, error_bitfield, &msg);
		CAN_enqueue(&msg);
		...
		for (uint8_t i = 0; i < channel_remote_count(); i++) {
			...
			build_telemetry_info_msg(
				PRIO_MEDIUM, (uint16_t)now, channel_remote_from_index(i), lqi, rssi, &msg);
			CAN_enqueue(&msg);
		}
